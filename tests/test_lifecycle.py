...
@pytest.mark.asyncio
async def test_razorpay_lifecycle_uses_authorized_then_captured(tmp_path: Path) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, request=request)
    ...
    assert [step.event for step in result.steps] == ["payment.authorized", "payment.captured"]
    assert all(step.acknowledged for step in result.steps)
